...
```bash
python3 char_demo.py --preset tiny-gpt --text-file data/tinyshakespeare/input.txt --max-chars 128 --validation-chars 32 --seed-text Firs --output-dir /tmp/nanollm-char-run
```
...
`--embedding-dim` unless intentionally overriding them. `--output-dir` writes
`model.json`, `metrics.csv`, and `samples.csv` for train/eval runs. Demo output
includes `sample dist-2`, the generated sample distinct-bigram ratio.
...
```bash
python3 char_demo.py --load-model /tmp/nanollm-char-run/model.json --text-file data/tinyshakespeare/input.txt --max-chars 128 --validation-chars 32 --eval-only --seed-text Firs
```
...
```bash
python3 char_demo.py --load-model /tmp/nanollm-char-run/model.json --generate-only --seed-text Firs --sample-mode sample --temperature 0.8 --top-k 8 --num-samples 3 --output-dir /tmp/nanollm-char-sample-run
```

Prompts can also come from a local file:

```bash
python3 char_demo.py --load-model /tmp/nanollm-char-run/model.json --generate-only --seed-file /tmp/prompt.txt --generate 80
```

Sweep checkpoint sampling settings into one CSV:

```bash
python3 char_sample_grid.py --load-model /tmp/nanollm-char-run/model.json --seed-text Firs --temperatures 0.7 0.8 1.0 --top-k none 4 8 --output-dir /tmp/nanollm-char-grid
```
