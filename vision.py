...
class ThreeConvCNN(TensorModule):
    """Three-convolution channel-first CNN for small CIFAR-style experiments."""
    ...
    def __init__(
        ...
        *,
        ...
        if len(image_shape) != 3:
            raise ValueError("ThreeConvCNN expects image_shape=(channels, rows, cols)")
        if classes <= 0:
            raise ValueError("classes must be positive")
        if filters <= 0 or second_filters <= 0 or third_filters <= 0:
            raise ValueError("filters must be positive")
        if kernel_size <= 0:
            raise ValueError("kernel_size must be positive")
        if pool_size <= 0:
        ...
        conv2_rows, conv2_cols = _valid_conv_shape(pool1_rows, pool1_cols, kernel_size)
        pool2_rows, pool2_cols = _pool_shape(conv2_rows, conv2_cols, pool_size)
        conv3_rows, conv3_cols = _valid_conv_shape(pool2_rows, pool2_cols, kernel_size)
        pool3_rows, pool3_cols = _pool_shape(conv3_rows, conv3_cols, pool_size)
        classifier_inputs = third_filters * pool3_rows * pool3_cols
        ...
        self.conv3 = TensorConv2D(
            (third_filters, second_filters, kernel_size, kernel_size),
            seed=seed + 2,
        )
        self.classifier = TensorLinear(
            inputs=classifier_inputs,
    ...
    def __call__(self, inputs: Tensor) -> Tensor:
        ...
        features = _activate(self.conv2(pooled), self.activation)
        pooled = _pool(features, self.pool_size, pooling=self.pooling)
        features = _activate(self.conv3(pooled), self.activation)
        pooled = _pool(features, self.pool_size, pooling=self.pooling)
        return self.classifier(pooled.flatten(start_axis=1))
